...
class LeaguesAdapter : BaseQuickAdapter<League, BaseViewHolder>(R.layout.item_league) {
    ...
    var onCurrentSelectedItemListener: ((id: Long) -> Unit)? = null
    ...
    override fun convert(helper: BaseViewHolder, item: League?) {
        item?.let { league ->
            ...
            helper.itemView.setOnClickListener {
                onItemClickListener?.invoke(helper.adapterPosition, league.id.toString())
                lastSelectedPosition = helper.adapterPosition
            }
        }
    }

    fun deleteItem(league: League) {
        data.remove(league)
    }

    fun getLeague(position: Int): League? {
